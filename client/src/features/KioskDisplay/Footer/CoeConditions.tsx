...
const CoeConditions = ({ temperature, humidity, co2 }: SensorDataType) => {
  return (
    <div className="relative flex flex-col items-center rounded-2xl shadow-lg h-full w-full px-1">
      ...
      <div className="relative z-10 p-4 rounded-2xl shadow-lg space-y-4">
        <Card
          label="Temperature"
          icon="🌡️"
          value={temperature}
...
function Card({ label, value, icon, type }: CardProps) {
  ...
  return (
    <div className="rounded-xl flex flex-col items-center justify-between text-white">
      ...
      <div className={`text-3xl font-bold ${getStatusColor(type, value)}`}>
        {value}
        {unitMap[type]}
      </div>
    </div>
