...
const MainAside = memo(({ isPortrait }: { isPortrait: boolean }) => {
  return (
    <>
      <div
        className={`${
          isPortrait ? "basis-1/2 w-full h-1/2" : "h-full flex-1"
      ...
      <div
        className={`${
          isPortrait ? "basis-1/2 w-full" : "h-full flex-1"
      ...
      {!isPortrait && (
        <div className="h-full flex-1 bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-xl overflow-hidden">
          <img
